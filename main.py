...
class Monomial:
  ...
  def __str__(self):
    variables = [ str(v) for v in self.variables if v.power > 0 ]
    if self.is_constant() or len(variables) == 0:
...
class Polynomial:
  ...
  def multivariate_dense_coefficients(self) -> DenseVector:
    d = self.degree()
    k = self.variables_count()
    coefficients = [0] * (d + 1)**k
    for p, c in self.multivariate_sparse_coefficients().items():
      coefficients[p] = c
    return coefficients

  def multivariate_sparse_coefficients(self) -> SparseVector:
    ...
    for m in self.monomials:
      ...
      coefficients[position] = m.coefficient
    return coefficients
...
def matrix_by_sparse_vector_mod_m(M: SparseMatrix, v: SparseVector, m: int) -> SparseVector:
  # Obtain the amount of rows in the sparse matrix
  Mr = len(list(M.values())[0])
...
def matrix_by_dense_vector_mod_m(M: DenseMatrix, v: DenseVector, m: int) -> DenseVector:
...
def matrix_kroneker_product_full_mod_m(M: DenseMatrix, k: int, m: int) -> DenseMatrix:
  Mr = len(M)
  Mc = len(M[0])
  T = M
  for _ in range(k - 1):
    ...
    for i in range(len(N)):
      for j in range(len(N[0])):
        N[i][j] = (T[i % Tr][j % Tc] * M[i // Tr][j // Tc]) % m
    T = N
  return T

def matrix_kroneker_product_part_mod_m(M: DenseMatrix, C: DenseVector, k: int, m: int) -> SparseMatrix:
  Mr = len(M)
  Mc = len(M[0])
  # Compute the columns we need at each Kronecker product step
  # k = 3, c^k = 5
  # k = 2, c^k = c^(k + 1) % (Mc**k) = 5 % 4 = 1
  KC = []
  for c, _ in C.items():
    indices = []
    for e in range(k, 1, -1):
      ...
      c = (c % Mc**(e - 1))
    KC.append(indices)
  # Initialize the base matrix columns
  PC = {}
  NC = {}
  for i in range(Mr):
    for j in range(Mc):
...
def matrix_inverse_mod_m(M: DenseMatrix, m: int) -> DenseMatrix:
  assert len(M) > 0
...
def matrix_extract(M: DenseMatrix, r: int, c: int) -> DenseMatrix:
...
def reduce_univariate_coefficients(v: DenseVector, b: int, w: int) -> DenseVector:
  r = [ a for a in v ]
  for j in range(2, b):
    r[j] %= cj_univariate(j, w)
  return r

def reduce_multivariate_dense_coefficients(v: DenseVector, d: int, k: int, w: int) -> DenseVector:
  r = [ a for a in v ]
...
def reduce_multivariate_sparse_coefficients(v: SparseVector, d: int, k: int, w: int) -> SparseVector:
  r = v
  for j, c in v.items():
    p = position_to_powers_vector(j, d, k)
    r[j] %= cj_multivariate(p, w)
...
def expand_univariate_coefficients(v: DenseVector, b: int, w: int) -> DenseVector:
  r = [ a for a in v ]
  for j in range(2, b):
    r[j] += sj(j, w) * cj_univariate(j, w)
  return r

def expand_multivariate_dense_coefficients(v: DenseVector, b: int, d: int, k: int, w: int) -> DenseVector:
  r = [ a for a in v ]
...
def expand_multivariate_sparse_coefficients(v: SparseVector, b: int, d: int, k: int, w: int) -> SparseVector:
  r = v
  for j in range(2, b**k):
    p = position_to_powers_vector(j, d, k)
    if j not in r:
...
def vector_to_univariate_polynomial(v: DenseVector) -> Polynomial:
  x = Variable("x")
...
def dense_vector_to_multivariate_polynomial(v: DenseVector, d: int, k: int) -> Polynomial:
...
def sparse_vector_to_multivariate_polynomial(v: SparseVector, d: int, k: int) -> Polynomial:
  # Generate the correct amount of variables
  x = {}
  for i in range(k):
    x[i] = Variable(f"x{i}")
...
def position_to_powers_vector(j: int, d: int, k: int) -> DenseVector:
    powers = []
...
def obtain_multivariate_factorial_dense_coefficients(v: DenseVector, d: int, k: int) -> Tuple[int, DenseVector]:
...
def obtain_multivariate_factorial_sparse_coefficients(v: SparseVector, d: int, k: int) -> Tuple[int, SparseVector]:
  # Obtain the powers, the coefficient and the maximum degree
  max_degree = 0
  coefficients_and_powers = []
  for j, coefficient in v.items():
    powers = position_to_powers_vector(j, d, k)
    coefficients_and_powers.append((coefficient, powers))
    for power in powers:
      if power > max_degree:
...
def sparse_to_dense_vector(v: SparseVector, s: int) -> DenseVector:
  d = [0] * s
  for j, c in v.items():
    d[j] = c
  return d

def sparse_to_dense_matrix(M: SparseMatrix, s: int) -> DenseMatrix:
  r = len(list(M.values())[0])
  c = s
...
def print_dense_matrix(M: DenseMatrix):
  for R in M:
    print("[", end="")
    for c in R:
      print(f"{c: 4d}", end="")
...
def print_sparse_matrix(M: SparseMatrix, s: int):
  print_dense_matrix(sparse_to_dense_matrix(M, s))
...
def cj_multivariate(j: DenseVector, w: int) -> int:
  e = w
  for i in j:
    e -= v2(i)
  if e < 0:
...
def F(C: DenseMatrix, r: int, c: int, w: int) -> DenseMatrix:
...
def normalize_univariate_polynomial(p: Polynomial, w: int, debug: bool) -> Polynomial:
  ...
  if debug:
    ...
    print_dense_matrix(MC)

  MF = F(MC, l, d + 1, w)

  if debug:
    print(f"F[{len(MF)}, {len(MF[0])}]:")
    print_dense_matrix(MF)
...
def normalize_multivariate_polynomial(p: Polynomial, w: int, dense: bool, debug: bool) -> Polynomial:
  ...
  if dense:
    ...
    v = p.multivariate_dense_coefficients()
    ...
    MFK = matrix_kroneker_product_full_mod_m(MF, k, 2**w)

    if debug:
      print(f"FK[{len(MFK)}, {len(MFK[0])}]:")
      print_dense_matrix(MFK)

    u = matrix_by_dense_vector_mod_m(MFK, v, 2**w)

    if debug:
      print(f"u: {u}")

    u = reduce_multivariate_dense_coefficients(u, d, k, w)

    if debug:
      print(f"u: {u}")

    dw, u = obtain_multivariate_factorial_dense_coefficients(u, d, k)
    ...
    MCK = matrix_kroneker_product_full_mod_m(MC, k, 2**w)

    if debug:
      print(f"CK[{len(MCK)}, {len(MCK[0])}]:")
      print_dense_matrix(MCK)

    v = matrix_by_dense_vector_mod_m(MCK, u, 2**w)

    if debug:
      print(f"v: {v}")

    return dense_vector_to_multivariate_polynomial(v, dw, k)

  else:

    v = p.multivariate_sparse_coefficients()

    if debug:
      print(f"d: {d}")
      print(f"k: {k}")
      print(f"v: {v}")
      print(f"v: {sparse_to_dense_vector(v, (d + 1)**k)}")
    ...
    if debug:
      print(f"u: {u}")
      print(f"u: {sparse_to_dense_vector(u, (d + 1)**k)}")

    u = reduce_multivariate_sparse_coefficients(u, d, k, w)

    if debug:
      print(f"u: {u}")
      print(f"u: {sparse_to_dense_vector(u, (d + 1)**k)}")

    dw, u = obtain_multivariate_factorial_sparse_coefficients(u, d, k)
    ...
    return sparse_vector_to_multivariate_polynomial(v, dw, k)

def equivalent_multivariate_polynomial(p: Polynomial, w: int, t: int, dense: bool, debug: bool) -> Polynomial:
  ...
  if dense:
    ...
    u = expand_multivariate_dense_coefficients(u, t + 1, t, k, w)

    if debug:
      print(f"u: {u}")

    dw, u = obtain_multivariate_factorial_dense_coefficients(u, t, k)
    ...
    if debug:
      print(f"u: {u}")
      print(f"u: {sparse_to_dense_vector(u, (t + 1)**k)}")

    u = expand_multivariate_sparse_coefficients(u, t + 1, t, k, w)

    if debug:
      print(f"u: {u}")
      print(f"u: {sparse_to_dense_vector(u, (t + 1)**k)}")

    dw, u = obtain_multivariate_factorial_sparse_coefficients(u, t, k)
...
multivariate_polynomials = [
  ...
  (8, x0**8*x1**8),
  (8, x0**8*x1**8*x2**8),
  (8, x0**8*x1**8*x2**8*x3**8),
  (8, x0**3*x1**3*x2**3*x3**3*x4**3),
  (8, x0**4*x1**4*x2**4*x3**4*x4**4*x5**4),
...
if __name__ == '__main__':
  # Parse the arguments
  parser = ArgumentParser(description="Efficient Normalized Reduction and Generation of Equivalent Multivariate Binary Polynomials")
  parser.add_argument("--debug", action=BooleanOptionalAction, help="Enable the debug logs")
  parser.add_argument("--dense", action=BooleanOptionalAction, help="Enable the use of dense vectors")
  parser.add_argument("--tests", action=BooleanOptionalAction, help="Enable the polynomial tests")
  parser.add_argument("--delta", type=int, help="Specify the EquivalentPolynomial delta")
  ...
  for w, p in univariate_polynomials:
    ...
    T1 = time()
    print(f"q = normalize_univariate_polynomial(p): {q}, T(q): {(T1 - T0) * 1000:.02f}ms")
    T0 = time()
    e = equivalent_univariate_polynomial(q, w, q.degree() + delta, arguments.debug)
    T1 = time()
    print(f"e = equivalent_univariate_polynomial(q, {q.degree() + delta}): {e}, T(e): {(T1 - T0) * 1000:.02f}ms")
    T0 = time()
    n = normalize_univariate_polynomial(e, w, arguments.debug)
    T1 = time()
    print(f"n = normalize_univariate_polynomial(e): {n}, T(n): {(T1 - T0) * 1000:.02f}ms")
    assert n == q
  ...
  for w, p in multivariate_polynomials:
    ...
    print(f"p: {p}")
    T0 = time()
    q = normalize_multivariate_polynomial(p, w, arguments.dense, arguments.debug)
    T1 = time()
    print(f"q = normalize_multivariate_polynomial(p): {q}, T(q): {(T1 - T0) * 1000:.02f}ms")
    T0 = time()
    e = equivalent_multivariate_polynomial(q, w, q.degree() + delta, arguments.dense, arguments.debug)
    T1 = time()
    print(f"e = equivalent_multivariate_polynomial(q, {q.degree() + delta}): {e}, T(e): {(T1 - T0) * 1000:.02f}ms")
    T0 = time()
    n = normalize_multivariate_polynomial(e, w, arguments.dense, arguments.debug)
    T1 = time()
    print(f"n = normalize_multivariate_polynomial(e): {n}, T(n): {(T1 - T0) * 1000:.02f}ms")
    assert n == q
  # Notify the end
  print("="*200)
